...
public class GeoHash36{
	...
	public static String encode(Coordinates coordinates_, int numCharacters_){
		...
		while(numCharacters_ > 0){
			...
			for(int i = 0; i < GEOHASH_MATRIX_SIDE; i++){
				...
				if((coordinates_.getLongitude() > leftBoundary) && (coordinates_.getLongitude() <= rightBoundary)){
					longIdx = i;
					lon[0] = leftBoundary;
					lon[1] = rightBoundary;
					break;
			...
			for(int i = 0; i < GEOHASH_MATRIX_SIDE; i++){
				...
				if((coordinates_.getLatitude() > leftBoundary) && (coordinates_.getLatitude() <= rightBoundary)){
	...
	public static Coordinates decode(String buffer_){
		...
		return Coordinates.createCoordinates((lat[1] + lat[0]) / 2, (lon[1] + lon[0]) / 2);
	}
	...
	public static Pair<Double, Double> getPrecisionInMeters(int numCharacters_){
		...
		final double latPrec = (90 / (double) fastPow(GEOHASH_MATRIX_SIDE, numCharacters_)) * one_grade_in_meters;
	 ...
	 * @return
	 */
	public static String getNeighbor(String buffer_, NeighborsDir direction_){
		final Pair<Integer, Integer> latLong = charToIndexes(buffer_.charAt(buffer_.length() - 1));
